...
int bitCount = 0; // The number of bits used for delta-sigma conversion
long bitSum = 0; // The sum of bits used for delta-sigma conversion
...
void loop() {
  ...
  // Measure the capacitor voltage using delta-sigma conversion
  bitCount++; // Increment the bit count
  if (analogRead(measurePin) > bitSum / bitCount) { // Compare the analog reading with the average bit value
    bitSum += 1023; // Add a high bit value to the sum
    capVoltage += (5.0 / bitCount); // Add a high bit value to the voltage
  }
